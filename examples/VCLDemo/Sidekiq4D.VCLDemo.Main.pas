...
type
  ...
  private
    FQueueTraditional: TSidekiqInMemoryQueueAdapter;
    FQueueSidekiq: TSidekiqInMemoryQueueAdapter;
    FRunning: Boolean;
    FTraditionalCount: Integer;
    FSidekiqCount: Integer;
    FTraditionalWatch: TStopwatch;
    FSidekiqWatch: TStopwatch;
...
type
  TDemoHandler = class(TInterfacedObject, ISidekiqJobHandler)
  private
    FWorkMs: Integer;
    FCounter: PInteger;
...
function TDemoHandler.CanHandle(const AJob: ISidekiqJobEnvelope): Boolean;
begin
  Result := True;
end;

procedure TDemoHandler.Perform(const AContext: ISidekiqJobContext);
var
  LCount: Integer;
begin
  Sleep(FWorkMs);
...
var
  ...
  LElapsed: Double;
begin
  LCount := FSidekiqCount;
  LElapsed := FSidekiqWatch.Elapsed.TotalSeconds;
  System.Classes.TThread.Queue(nil,
    procedure
    begin
      if LElapsed > 0 then
...
begin
  ...
  FQueueTraditional := TSidekiqInMemoryQueueAdapter.New;
  FQueueSidekiq := TSidekiqInMemoryQueueAdapter.New;
  ...
  begin
    ...
    FQueueSidekiq.Enqueue('process_nfce', LBody);
  end;
end;
...
begin
  ...
  LogRight(Format('[SEED] %d mensagens na fila (Sidekiq4D)', [FTotalMessages]));
end;
...
begin
  ...
  try
    ...
    LogRight(Format('[DONE] %d msgs em %.1fs (%.1f msgs/s)',
      [FSidekiqCount, FSidekiqWatch.Elapsed.TotalSeconds,
       FSidekiqCount / FSidekiqWatch.Elapsed.TotalSeconds]));
    UpdateStatusRight;
