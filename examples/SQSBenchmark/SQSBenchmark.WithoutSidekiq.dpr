program SQSBenchmarkWithoutSidekiq;

{$APPTYPE CONSOLE}

// Benchmark: Consumo SQS SEM Sidekiq4D (loop manual - padrao legado)
//
...
var
  SQS: TSidekiqSqsClient;
  Watch: TStopwatch;
  TotalProcessed: Integer;
  EmptyCycles: Integer;
  Options: TSidekiqSqsReceiveOptions;
  Messages: TObjectList<TSidekiqSqsMessage>;
  Msg: TSidekiqSqsMessage;
begin
  CoInitialize(nil);
  try
    WriteLn('=== SQS Benchmark: SEM Sidekiq4D (loop manual) ===');
    WriteLn('');
    ...
    try
      ...
      WriteLn('');
      WriteLn('=== Resultado SEM Sidekiq4D ===');
