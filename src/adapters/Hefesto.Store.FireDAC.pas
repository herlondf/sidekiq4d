...
type
  // Backend FireDAC generico para IHefestoPostgresStateStoreBackend.
  // Funciona com Postgres, SQLite, MySQL, SQL Server e qualquer driver FireDAC.
  //
  // Uso com Postgres:
  //   THefestoPostgresStateStore.New
  ...
  THefestoFireDACBackend = class(TInterfacedObject, IHefestoPostgresStateStoreBackend)
  private
  ...
  public
    ...
    class function New(const AConnection: TFDConnection;
      AOwnsConnection: Boolean = False): IHefestoPostgresStateStoreBackend; static;
    ...
    function ListEntries(const ATableName, APrefix: string): TArray<THefestoPostgresStateEntry>;
...
{ THefestoFireDACBackend }

constructor THefestoFireDACBackend.Create(
...
destructor THefestoFireDACBackend.Destroy;
begin
...
class function THefestoFireDACBackend.New(
  const AConnection: TFDConnection;
  AOwnsConnection: Boolean): IHefestoPostgresStateStoreBackend;
begin
  Result := THefestoFireDACBackend.Create(AConnection, AOwnsConnection);
end;

class function THefestoFireDACBackend.NewSQLiteConnection(
...
class function THefestoFireDACBackend.NewPostgresConnection(
  const AHost: string; APort: Integer;
...
procedure THefestoFireDACBackend.EnsureTable(const ATableName: string);
var
  LQualified: string;
begin
...
procedure THefestoFireDACBackend.PurgeExpiredIfDue(const ATableName: string);
const
...
function THefestoFireDACBackend.GetEntry(const ATableName, AKey: string;
  out AEntry: THefestoPostgresStateEntry): Boolean;
var
  LQuery: TFDQuery;
begin
...
procedure THefestoFireDACBackend.PutEntry(const ATableName: string;
  const AEntry: THefestoPostgresStateEntry);
var
  LExpiresAt: string;
  LSQL: string;
...
procedure THefestoFireDACBackend.DeleteEntry(const ATableName, AKey: string);
begin
  FLock.Acquire;
  try
    EnsureTable(ATableName);
...
function THefestoFireDACBackend.ListEntries(
  const ATableName, APrefix: string): TArray<THefestoPostgresStateEntry>;
var
  LQuery: TFDQuery;
  LList: TArray<THefestoPostgresStateEntry>;
  LCount: Integer;
begin
  FLock.Acquire;
  try
...
function THefestoFireDACBackend.TryPutIfAbsentOrExpired(
  const ATableName: string;
  const AEntry: THefestoPostgresStateEntry;
  const AReferenceTime: TDateTime): Boolean;
var
  LExisting: THefestoPostgresStateEntry;
  LFound: Boolean;
  LQuery: TFDQuery;
  LExpiresAt: string;
  LSQL: string;
