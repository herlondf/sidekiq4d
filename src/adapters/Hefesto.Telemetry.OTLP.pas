unit Hefesto.Telemetry.OTLP;

{
  OpenTelemetry Trace Exporter para Hefesto.

  Implementa IHefestoTelemetry e exporta spans OTLP/HTTP JSON para qualquer
  ...
  Ou combinado com o console via THefestoCompositeTelemetry:
    Server.UseTelemetry(
...
type
  THefestoOTLPTraceTelemetry = class(TInterfacedObject, IHefestoTelemetry)
  private type
    TSpanContext = record
      TraceId   : string;
      SpanId    : string;
  ...
  public
    ...
    class function New(
      const AEndpoint    : string;
      const AServiceName : string;
      const AInstanceId  : string = ''): IHefestoTelemetry;
...
{ THefestoOTLPTraceTelemetry — private helpers }

class function THefestoOTLPTraceTelemetry.GenerateTraceId: string;
var
  LG1, LG2: TGuid;
begin
  CreateGUID(LG1);
...
class function THefestoOTLPTraceTelemetry.GenerateSpanId: string;
var
  LG: TGuid;
begin
  CreateGUID(LG);
...
class function THefestoOTLPTraceTelemetry.NowUnixNano: Int64;
var
  LSeconds: Int64;
  LFrac   : Int64;
begin
...
{ THefestoOTLPTraceTelemetry }

constructor THefestoOTLPTraceTelemetry.Create(
...
destructor THefestoOTLPTraceTelemetry.Destroy;
begin
  FLock.Free;
  FPendingSpans.Free;
  inherited;
...
class function THefestoOTLPTraceTelemetry.New(
  const AEndpoint, AServiceName, AInstanceId: string): IHefestoTelemetry;
begin
  Result := THefestoOTLPTraceTelemetry.Create(AEndpoint, AServiceName, AInstanceId);
end;

procedure THefestoOTLPTraceTelemetry.ExportSpan(
  const AJobId   : string;
  const ACtx     : TSpanContext;
  const AEndNano : Int64;
  const ASuccess : Boolean;
...
procedure THefestoOTLPTraceTelemetry.CloseSpan(
  const AJob    : IHefestoJobEnvelope;
  const ASuccess: Boolean;
  const AErrorMsg: string);
var
...
{ IHefestoTelemetry — stubs para eventos sem span }

procedure THefestoOTLPTraceTelemetry.ServerStarted;   begin end;
procedure THefestoOTLPTraceTelemetry.ServerStopped;   begin end;
procedure THefestoOTLPTraceTelemetry.PollingStarted;  begin end;
procedure THefestoOTLPTraceTelemetry.PollingStopped;  begin end;
procedure THefestoOTLPTraceTelemetry.Idle(const ADelayMs: Integer); begin end;
procedure THefestoOTLPTraceTelemetry.FetchFinished(const AQueueName: string; const ACount: Integer); begin end;
procedure THefestoOTLPTraceTelemetry.JobAcked(const AJob: IHefestoJobEnvelope); begin end;
procedure THefestoOTLPTraceTelemetry.JobDiscarded(const AJob: IHefestoJobEnvelope; const AReason: string); begin end;

{ IHefestoTelemetry — ciclo de vida do span }

procedure THefestoOTLPTraceTelemetry.JobStarted(const AJob: IHefestoJobEnvelope);
var
  LCtx: TSpanContext;
begin
...
procedure THefestoOTLPTraceTelemetry.JobSucceeded(
  const AJob: IHefestoJobEnvelope;
...
procedure THefestoOTLPTraceTelemetry.JobFailed(
  const AJob: IHefestoJobEnvelope;
...
procedure THefestoOTLPTraceTelemetry.JobRetried(
  const AJob: IHefestoJobEnvelope;
...
procedure THefestoOTLPTraceTelemetry.JobDeadLettered(
  const AJob: IHefestoJobEnvelope;
