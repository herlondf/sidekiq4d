unit Hefesto.Redis.Client;

// Minimal Redis client interface used by all Hefesto Redis adapters.
// Zero third-party dependencies — implement with any Redis library.
// See Hefesto.Redis4D.Client for the bundled Redis4D bridge.
...
type
  ...
  THefestoStreamField = record
    Name: string;
    Value: string;
  end;

  // Message returned by IHefestoRedisStreams.ReadGroup
  THefestoStreamMessage = record
    ID: string;
    Fields: TArray<THefestoStreamField>;
  ...
  IHefestoRedisStreams = interface
  ...
  IHefestoRedisPipeline = interface
    ['{C3D7E1F0-A2B4-4C6D-8E9F-0A1B2C3D4E5F}']
    function LPush(const AKey, AValue: string): IHefestoRedisPipeline;
    function ZAdd(const AKey: string; AScore: Double; const AMember: string): IHefestoRedisPipeline;
    // Queue any raw Redis command (e.g. XADD, INCR, DEL …)
    function QueueCommand(const ACommand: TArray<string>): IHefestoRedisPipeline;
    // Fire all queued commands in one round-trip. Safe to call with 0 commands (no-op).
    procedure Execute;
  end;

  // Minimal Redis client interface — all operations used by Hefesto adapters.
  // Covers KV, sorted sets, lists, Lua scripting and Streams.
  IHefestoRedisClient = interface
...
{ THefestoStreamMessage }

function THefestoStreamMessage.FieldValue(const AName: string): string;
var
  LField: THefestoStreamField;
begin
  Result := '';
...
function THefestoStreamMessage.HasField(const AName: string): Boolean;
begin
  Result := IndexOfField(AName) >= 0;
end;

function THefestoStreamMessage.IndexOfField(const AName: string): Integer;
var
  LIndex: Integer;
begin
