unit Hefesto.Web.Indy;

{
  HefestoWeb4D — Dashboard HTTP embutido (v2), implementado com Indy TIdHTTPServer.
  ...
  Endpoints REST:
    ...
    GET    /api/scheduled             — jobs no IHefestoScheduledStore
...
type
  ...
  THefestoWsBroadcastThread = class(TThread)
  private
  ...
  public
    constructor Create(
      const AHub    : IHefestoWebSocketHub;
      const AGetJson: TFunc<string>);
  end;

  THefestoIndyWebDashboard = class(TInterfacedObject, IHefestoWebDashboard)
  private
    FServer          : TIdHTTPServer;
    FConfig          : THefestoWebDashboardConfig;
    FLock            : TCriticalSection;
    FRunning         : Boolean;
    FStartTime       : TDateTime;
    FWsHub            : IHefestoWebSocketHub;
    FWsBroadcastThread: THefestoWsBroadcastThread;
  ...
  public
    constructor Create(const AConfig: THefestoWebDashboardConfig);
    destructor Destroy; override;

    class function New(
      const AConfig: THefestoWebDashboardConfig): IHefestoWebDashboard;

    { IHefestoWebDashboard }
...
{ THefestoIndyWebDashboard }

{ THefestoWsBroadcastThread }

constructor THefestoWsBroadcastThread.Create(
  const AHub    : IHefestoWebSocketHub;
  const AGetJson: TFunc<string>);
begin
...
procedure THefestoWsBroadcastThread.Execute;
begin
...
constructor THefestoIndyWebDashboard.Create(
  const AConfig: THefestoWebDashboardConfig);
begin
  inherited Create;
  FConfig             := AConfig;
  ...
  FRunning            := False;
  FStartTime          := Now;
  FWsHub              := THefestoInMemoryWebSocketHub.New;
  FServer             := TIdHTTPServer.Create(nil);
  FServer.OnCommandGet   := HandleCommand;
  FServer.OnCommandOther := HandleCommand;
  FWsBroadcastThread  := THefestoWsBroadcastThread.Create(
    FWsHub,
    function: string
    begin
...
destructor THefestoIndyWebDashboard.Destroy;
begin
  Stop;
...
class function THefestoIndyWebDashboard.New(
  const AConfig: THefestoWebDashboardConfig): IHefestoWebDashboard;
begin
  Result := THefestoIndyWebDashboard.Create(AConfig);
end;

procedure THefestoIndyWebDashboard.Start(const APort: Word);
begin
  FLock.Acquire;
  try
    if FRunning then
...
procedure THefestoIndyWebDashboard.Stop;
begin
  FLock.Acquire;
  try
...
function THefestoIndyWebDashboard.IsRunning: Boolean;
begin
  FLock.Acquire;
  try
    Result := FRunning;
...
procedure THefestoIndyWebDashboard.HandleCommand(
  AContext     : TIdContext;
  ARequestInfo : TIdHTTPRequestInfo;
  AResponseInfo: TIdHTTPResponseInfo);
var
...
function THefestoIndyWebDashboard.HandleApiOverview: string;
var
  LObj       : TJSONObject;
  LWorkers   : Integer;
  LMaxConc   : Integer;
  ...
  LDepth     : IHefestoQueueDepth;
  LSnaps     : TArray<THefestoHistoricalMetricSnapshot>;
  LSnap      : THefestoHistoricalMetricSnapshot;
begin
  LWorkers    := 0;
  LMaxConc    := 0;
  LIsLeader   := False;
  ...
  for LQueue in FConfig.Queues do
    if Supports(LQueue, IHefestoQueueDepth, LDepth) then
...
function THefestoIndyWebDashboard.HandleApiQueues: string;
var
  LArr     : TJSONArray;
  LObj     : TJSONObject;
  LQueue   : IHefestoQueueAdapter;
...
function THefestoIndyWebDashboard.HandleApiWorkers: string;
var
  LArr   : TJSONArray;
  LObj   : TJSONObject;
  LKeys  : TArray<string>;
...
function THefestoIndyWebDashboard.HandleApiScheduled: string;
var
  LArr    : TJSONArray;
  LObj    : TJSONObject;
...
function THefestoIndyWebDashboard.HandleApiPeriodic: string;
var
  LArr : TJSONArray;
  LObj : TJSONObject;
  LJobs: TArray<THefestoPeriodicDefinition>;
  LJob : THefestoPeriodicDefinition;
begin
  LArr := TJSONArray.Create;
  try
    if Assigned(FConfig.DashboardServer) then
...
function THefestoIndyWebDashboard.HandleApiBatches: string;
var
  LArr    : TJSONArray;
  LObj    : TJSONObject;
  LKeys   : TArray<string>;
...
function THefestoIndyWebDashboard.HandleApiMetrics: string;
var
  LArr  : TJSONArray;
  LObj  : TJSONObject;
  LSnaps: TArray<THefestoHistoricalMetricSnapshot>;
...
function THefestoIndyWebDashboard.HandleApiLocks: string;
var
  LArr : TJSONArray;
  LObj : TJSONObject;
  LKeys: TArray<string>;
...
function THefestoIndyWebDashboard.HandleApiDlq(
  const AAction, AQueue, AFrom, ATo: string): string;
var
  LArr       : TJSONArray;
  LObj       : TJSONObject;
  LEntries   : TArray<THefestoDeadLetterEntry>;
...
function THefestoIndyWebDashboard.HandleApiQueuePause(
...
function THefestoIndyWebDashboard.HandleApiQueueResume(
  const AName: string): Integer;
begin
  if AName.IsEmpty or not Assigned(FConfig.DashboardServer) then
    Exit(400);
...
function THefestoIndyWebDashboard.HandleApiDlqRetryAll: Integer;
var
  LEntries: TArray<THefestoDeadLetterEntry>;
  LEntry  : THefestoDeadLetterEntry;
begin
...
function THefestoIndyWebDashboard.HandleApiDlqRetryFiltered(
...
function THefestoIndyWebDashboard.HandleApiDlqClear: Integer;
var
  LEntries: TArray<THefestoDeadLetterEntry>;
  LEntry  : THefestoDeadLetterEntry;
begin
...
function THefestoIndyWebDashboard.HandleApiDlqRetry(
...
function THefestoIndyWebDashboard.HandleApiDlqDelete(
  const AJobId: string): Integer;
begin
  if AJobId.IsEmpty or not Assigned(FConfig.DeadLetterQueue) then
    Exit(400);
...
function THefestoIndyWebDashboard.HandleApiHealth: string;
begin
  Result := Format('{"status":"ok","uptime":%d}',
    [SecondsBetween(Now, FStartTime)]);
end;

function THefestoIndyWebDashboard.HandleApiPrometheusMetrics: string;
var
...
function THefestoIndyWebDashboard.HandleApiScheduledDelete(
  const AQueue, AAction, ADueAt: string): Integer;
var
  LParts: TArray<string>;
  LDate : TArray<string>;
...
class function THefestoIndyWebDashboard.ComputeWsAccept(
...
procedure THefestoIndyWebDashboard.HandleWebSocketUpgrade(
  AContext    : TIdContext;
  ARequestInfo: TIdHTTPRequestInfo);
var
  LKey   : string;
...
class function THefestoIndyWebDashboard.JsonEscape(
...
class function THefestoIndyWebDashboard.DateToIso(
...
class function THefestoIndyWebDashboard.FailedAtToStr(
  const AValue: TDateTime): string;
begin
  if AValue = 0 then
    Result := ''
...
class function THefestoIndyWebDashboard.ExtractSegment(
  const APath : string;
...
class function THefestoIndyWebDashboard.BuildHtml: string;
var
  LB: TStringBuilder;
begin
  ...
  try
    ...
    LB.Append('<title>Hefesto Dashboard</title>');
