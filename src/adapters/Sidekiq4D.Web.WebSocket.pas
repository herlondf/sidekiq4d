unit Sidekiq4D.Web.WebSocket;

{
  TSidekiqInMemoryWebSocketHub — Hub thread-safe para clientes WebSocket.
...
type
  TSidekiqInMemoryWebSocketHub = class(TInterfacedObject, ISidekiqWebSocketHub)
  private
...
{ TSidekiqInMemoryWebSocketHub }

constructor TSidekiqInMemoryWebSocketHub.Create;
begin
  inherited Create;
...
destructor TSidekiqInMemoryWebSocketHub.Destroy;
begin
  FClients.Free;
  FLock.Free;
  inherited;
...
class function TSidekiqInMemoryWebSocketHub.New: ISidekiqWebSocketHub;
begin
  Result := TSidekiqInMemoryWebSocketHub.Create;
end;

procedure TSidekiqInMemoryWebSocketHub.RegisterClient(
...
class function TSidekiqInMemoryWebSocketHub.BuildTextFrame(
...
procedure TSidekiqInMemoryWebSocketHub.BroadcastMetrics(const AJson: string);
var
  LFrame   : TBytes;
  LIdFrame : TIdBytes;
...
function TSidekiqInMemoryWebSocketHub.ConnectedClients: Integer;
begin
  FLock.Acquire;
  try
