unit Hefesto.Outbox.FireDAC;

{
  Transactional Outbox backed by FireDAC (Postgres, SQLite, MySQL, etc.).

  ATENÇÃO — Threading model:
    • THefestoFireDACOutbox.Save() usa a CONEXÃO DO CHAMADOR e participa
      da transação ativa nela. Não abre nem faz commit de transação.
    • THefestoFireDACOutboxPoller usa uma CONEXÃO EXCLUSIVA do thread de
...
type
  THefestoFireDACOutbox = class(TInterfacedObject, IHefestoOutbox)
  private
  ...
  public
    ...
    class function New(
      const ATableName: string = 'sidekiq_outbox'): IHefestoOutbox;
  ...
  public
    constructor Create(
      ...
      const APublisher     : IHefestoQueuePublisher;
    ...
    class function New(
      ...
      const ATableName     : string  = 'sidekiq_outbox';
      const APollIntervalMs: Integer = 1000;
      const ABatchSize     : Integer = 100): IHefestoOutboxPoller;

    { IHefestoOutboxPoller }
    procedure Start;
    procedure Stop;
...
{ THefestoFireDACOutbox }

constructor THefestoFireDACOutbox.Create(const ATableName: string);
begin
  inherited Create;
...
destructor THefestoFireDACOutbox.Destroy;
begin
  FLock.Free;
  inherited;
end;

class function THefestoFireDACOutbox.New(
  const ATableName: string): IHefestoOutbox;
begin
  Result := THefestoFireDACOutbox.Create(ATableName);
end;

procedure THefestoFireDACOutbox.EnsureTable(const AConn: TFDConnection);
begin
...
procedure THefestoFireDACOutbox.Save(
  const AQueueName : string;
  const AAction    : string;
  const APayload   : string;
  const AConnection: TObject);
...
begin
  if not (AConnection is TFDConnection) then
    raise EInvalidCast.Create(
      'THefestoFireDACOutbox.Save: AConnection must be a TFDConnection');
...
{ THefestoFireDACOutboxPoller }

constructor THefestoFireDACOutboxPoller.Create(
  const APollConnection: TFDConnection;
  const APublisher     : IHefestoQueuePublisher;
...
destructor THefestoFireDACOutboxPoller.Destroy;
begin
  Stop;
  FStopEvent.Free;
  FLock.Free;
...
class function THefestoFireDACOutboxPoller.New(
  const APollConnection: TFDConnection;
  const APublisher     : IHefestoQueuePublisher;
  const ATableName     : string;
  const APollIntervalMs: Integer;
  const ABatchSize     : Integer): IHefestoOutboxPoller;
begin
  Result := THefestoFireDACOutboxPoller.Create(
    APollConnection, APublisher, ATableName, APollIntervalMs, ABatchSize);
end;

procedure THefestoFireDACOutboxPoller.Start;
var
  LThis: THefestoFireDACOutboxPoller;
begin
  FLock.Acquire;
  try
    if FRunning then
...
procedure THefestoFireDACOutboxPoller.Stop;
var
  LThread: TThread;
begin
  FLock.Acquire;
...
procedure THefestoFireDACOutboxPoller.RunLoop;
begin
...
function THefestoFireDACOutboxPoller.FetchAndPublishBatch(
...
procedure THefestoFireDACOutboxPoller.MarkPublished(
  const AConn: TFDConnection;
  const AIds : TArray<string>);
var
  LNow : string;
...
function THefestoFireDACOutboxPoller.PollOnce: Integer;
begin
  if not FPollConnection.Connected then
    FPollConnection.Open;
