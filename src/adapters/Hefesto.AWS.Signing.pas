...
type
  ...
  public
    ...
    class function New: THefestoAwsV4Signer;

    function AccessKey(const AValue: string): THefestoAwsV4Signer;
    function SecretKey(const AValue: string): THefestoAwsV4Signer;
    function Region(const AValue: string): THefestoAwsV4Signer;
    function Service(const AValue: string): THefestoAwsV4Signer;
    function SessionToken(const AValue: string): THefestoAwsV4Signer;

    procedure Sign(const ARequest: THefestoHttpRequest);
  end;
...
function THefestoAwsV4Signer.AccessKey(
  const AValue: string): THefestoAwsV4Signer;
begin
  Result := Self;
  FAccessKey := AValue.Trim;
end;

class function THefestoAwsV4Signer.BuildCanonicalHeaders(
  const AHeaders: TDictionary<string, string>;
...
class function THefestoAwsV4Signer.BuildCanonicalQuery(
...
class function THefestoAwsV4Signer.BuildSigningKey(
...
constructor THefestoAwsV4Signer.Create;
begin
  inherited Create;
  FRegion := 'us-east-1';
  FService := 'sqs';
...
class function THefestoAwsV4Signer.HashSHA256Hex(const AValue: string): string;
begin
  Result := THashSHA2.GetHashString(AValue, THashSHA2.TSHA2Version.SHA256);
end;

class function THefestoAwsV4Signer.HmacSHA256(
  const AData: string;
...
class function THefestoAwsV4Signer.New: THefestoAwsV4Signer;
begin
  Result := THefestoAwsV4Signer.Create;
end;

class function THefestoAwsV4Signer.NormalizeHeaderValue(
...
class function THefestoAwsV4Signer.PercentEncode(const AValue: string): string;
const
...
function THefestoAwsV4Signer.Region(const AValue: string): THefestoAwsV4Signer;
begin
  Result := Self;
...
function THefestoAwsV4Signer.SecretKey(
  const AValue: string): THefestoAwsV4Signer;
begin
  Result := Self;
...
function THefestoAwsV4Signer.Service(const AValue: string): THefestoAwsV4Signer;
begin
  Result := Self;
  if not AValue.Trim.IsEmpty then
...
function THefestoAwsV4Signer.SessionToken(
  const AValue: string): THefestoAwsV4Signer;
begin
  Result := Self;
...
procedure THefestoAwsV4Signer.Sign(const ARequest: THefestoHttpRequest);
var
  LUri: TURI;
  LNowUtc: TDateTime;
  LAmzDate: string;
