...
type
  THefestoKafkaAdapter = class(TInterfacedObject, IHefestoQueueAdapter, IHefestoQueuePublisher)
  private
  ...
  public
    ...
    class function New: THefestoKafkaAdapter; static;

    function BaseUrl(const AValue: string): THefestoKafkaAdapter;
    function GroupId(const AValue: string): THefestoKafkaAdapter;
    function Topic(const AValue: string): THefestoKafkaAdapter;
    function InstanceId(const AValue: string): THefestoKafkaAdapter;
    function DeadLetterTopic(const AValue: string): THefestoKafkaAdapter;
...
{ THefestoKafkaAdapter }

constructor THefestoKafkaAdapter.Create;
begin
  inherited Create;
...
destructor THefestoKafkaAdapter.Destroy;
begin
  FLock.Free;
  inherited;
end;

class function THefestoKafkaAdapter.New: THefestoKafkaAdapter;
begin
  Result := THefestoKafkaAdapter.Create;
end;

function THefestoKafkaAdapter.BaseUrl(const AValue: string): THefestoKafkaAdapter;
begin
  Result := Self;
  FBaseUrl := AValue.TrimRight(['/']);
end;

function THefestoKafkaAdapter.GroupId(const AValue: string): THefestoKafkaAdapter;
begin
  Result := Self;
  FGroupId := AValue.Trim;
end;

function THefestoKafkaAdapter.Topic(const AValue: string): THefestoKafkaAdapter;
begin
  Result := Self;
  FTopic := AValue.Trim;
end;

function THefestoKafkaAdapter.InstanceId(const AValue: string): THefestoKafkaAdapter;
begin
  Result := Self;
  FInstanceId := AValue.Trim;
end;

function THefestoKafkaAdapter.DeadLetterTopic(const AValue: string): THefestoKafkaAdapter;
begin
  Result := Self;
  FDeadLetterTopic := AValue.Trim;
end;

function THefestoKafkaAdapter.ConsumerBaseUrl: string;
begin
  Result := Format('%s/consumers/%s/instances/%s', [FBaseUrl, FGroupId, FInstanceId]);
end;

function THefestoKafkaAdapter.CreateHttpClient: THTTPClient;
begin
  Result := THTTPClient.Create;
end;

function THefestoKafkaAdapter.DoGet(const AUrl: string): string;
var
  LClient: THTTPClient;
...
function THefestoKafkaAdapter.DoPost(const AUrl, ABody, AContentType: string): string;
var
  LClient: THTTPClient;
  LResponse: IHTTPResponse;
...
procedure THefestoKafkaAdapter.DoDelete(const AUrl: string);
var
  LClient: THTTPClient;
begin
  LClient := CreateHttpClient;
...
procedure THefestoKafkaAdapter.EnsureConsumerExists;
var
  LUrl: string;
  LBody: string;
begin
...
procedure THefestoKafkaAdapter.EnsureSubscription;
var
  LUrl: string;
  LBody: string;
begin
...
function THefestoKafkaAdapter.Name: string;
begin
  Result := 'kafka';
end;

function THefestoKafkaAdapter.Fetch(const AOptions: THefestoFetchOptions): TArray<IHefestoJobEnvelope>;
var
  LUrl: string;
  LResponse: string;
...
procedure THefestoKafkaAdapter.Ack(const AJob: IHefestoJobEnvelope);
var
  LUrl: string;
  LBody: string;
...
procedure THefestoKafkaAdapter.Nack(const AJob: IHefestoJobEnvelope; const ADelaySeconds: Integer);
begin
...
procedure THefestoKafkaAdapter.MoveToDeadLetter(const AJob: IHefestoJobEnvelope; const AReason: string);
var
...
procedure THefestoKafkaAdapter.Publish(const ARequest: THefestoPublishRequest);
var
  LUrl: string;
  LTargetTopic: string;
  LJson: TJSONObject;
...
function THefestoKafkaAdapter.ParseActionFromBody(const ABody: string): string;
var
  LJsonValue: TJSONValue;
