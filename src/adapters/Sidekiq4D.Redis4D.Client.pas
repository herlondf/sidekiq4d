unit Sidekiq4D.Redis4D.Client;

// Bridge: implements ISidekiqRedisClient using the Redis4D library.
//
// THIS IS THE ONLY FILE IN SIDEKIQ4D THAT IMPORTS Redis4D.*.
// All other adapters depend solely on ISidekiqRedisClient.
//
// Usage:
//   uses Sidekiq4D.Redis4D.Client;
//
//   // Standard connection
...
type
  // Base class — all ISidekiqRedisClient methods delegate to GetClient.
  TRedis4DClientBridgeBase = class(TInterfacedObject, ISidekiqRedisClient)
  private
    FStreams: ISidekiqRedisStreams;
  protected
  ...
  // Pool-backed bridge: each ISidekiqRedisClient call borrows a connection
  // from IRedis4DPool and returns it immediately after the call.
  // Use this when multiple threads share one ISidekiqRedisClient concurrently,
  ...
  TRedis4DPooledClientBridge = class(TInterfacedObject, ISidekiqRedisClient)
  private
    FPool: IRedis4DPool;
  public
    ...
    class function NewWithPool(
      ...
      APoolSize: Integer = 8;
      AWarmup: Integer = 2): ISidekiqRedisClient; static;
  ...
  public
    ...
    class function NewFromConnectionString(
      const AConnectionString: string): ISidekiqRedisClient; static;
  ...
  public
    ...
    class function New(
      ...
      const AServiceName: string;
      const APassword: string = '';
      ADatabase: Integer = 0): ISidekiqRedisClient; static;
  end;
...
type
  TRedis4DStreamsBridge = class(TInterfacedObject, ISidekiqRedisStreams)
  private
    FClient: IRedis4DClient;
...
type
  ...
  TRedis4DStreamsBridgePooled = class(TInterfacedObject, ISidekiqRedisStreams)
  private
    FInner: ISidekiqRedisStreams;
    FPool: IRedis4DPool;
    FClient: IRedis4DClient;
  public
    constructor Create(
...
type
  ...
  TSidekiqRedisPipelineImpl = class(TInterfacedObject, ISidekiqRedisPipeline)
  private
    FClient: IRedis4DClient;
  ...
  public
    ...
    function ZAdd(
      ...
      const AMember: string): ISidekiqRedisPipeline;
    function QueueCommand(
      const ACommand: TArray<string>): ISidekiqRedisPipeline;
    procedure Execute;
  end;

constructor TSidekiqRedisPipelineImpl.Create(
  const AClient: IRedis4DClient;
  const AOnRelease: TProc<IRedis4DClient>);
begin
  inherited Create;
...
destructor TSidekiqRedisPipelineImpl.Destroy;
begin
...
procedure TSidekiqRedisPipelineImpl.ReleaseClient;
begin
...
function TSidekiqRedisPipelineImpl.LPush(
  const AKey, AValue: string): ISidekiqRedisPipeline;
begin
  Result := Self;
  FCommands.Add(['LPUSH', AKey, AValue]);
end;

function TSidekiqRedisPipelineImpl.ZAdd(
  const AKey: string; AScore: Double;
  const AMember: string): ISidekiqRedisPipeline;
begin
  Result := Self;
...
function TSidekiqRedisPipelineImpl.QueueCommand(
  const ACommand: TArray<string>): ISidekiqRedisPipeline;
begin
  Result := Self;
...
procedure TSidekiqRedisPipelineImpl.Execute;
begin
  if FExecuted then
    Exit;
  FExecuted := True;
...
function TRedis4DClientBridgeBase.BeginPipeline: ISidekiqRedisPipeline;
begin
  Result := TSidekiqRedisPipelineImpl.Create(GetClient, nil);
end;
...
function TRedis4DClientBridgeBase.Streams: ISidekiqRedisStreams;
begin
...
class function TRedis4DClientBridge.NewFromConnectionString(
  const AConnectionString: string): ISidekiqRedisClient;
var
  LHost: string;
  LPort: Word;
  LPassword: string;
...
class function TRedis4DSentinelClientBridge.New(
  ...
  const AServiceName: string;
  const APassword: string;
  ADatabase: Integer): ISidekiqRedisClient;
begin
...
class function TRedis4DPooledClientBridge.NewWithPool(
  const AConnectionString: string;
  APoolSize: Integer;
  AWarmup: Integer): ISidekiqRedisClient;
var
  LHost: string;
  LPort: Word;
  LPassword: string;
...
function TRedis4DPooledClientBridge.BeginPipeline: ISidekiqRedisPipeline;
var
  LClient: IRedis4DClient;
  LPool: IRedis4DPool;
begin
  LClient := FPool.Acquire;
  LPool := FPool;
  Result := TSidekiqRedisPipelineImpl.Create(
    LClient,
...
function TRedis4DPooledClientBridge.Streams: ISidekiqRedisStreams;
var
  LClient: IRedis4DClient;
  LPool: IRedis4DPool;
begin
  // Acquire a dedicated connection for the lifetime of the returned
  // ISidekiqRedisStreams object. The connection is returned to the pool
  // when the ISidekiqRedisStreams reference is released (ref-count drops to 0).
  LClient := FPool.Acquire;
  LPool   := FPool;
