...
type
  EHefestoPeriodic = class(Exception);

  THefestoPeriodicDefinition = record
    Name: string;
...
function MakePeriodicDefinition(
  ...
  const AAttributes: TStrings): THefestoPeriodicDefinition;
...
begin
  if AName.Trim.IsEmpty then
    raise EHefestoPeriodic.Create('Periodic job name is required.');
  if ACronExpression.Trim.IsEmpty then
    raise EHefestoPeriodic.Create('Periodic cron expression is required.');
  if AAction.Trim.IsEmpty then
    raise EHefestoPeriodic.Create('Periodic action is required.');

  THefestoCronSchedule.Matches(
    ACronExpression,
    THefestoCronSchedule.TruncateToMinute(Now));
...
{ THefestoCronSchedule }

class function THefestoCronSchedule.MatchField(
  const AField: string;
...
class function THefestoCronSchedule.MatchSegment(
  const ASegment: string;
  const AValue, AMin, AMax: Integer): Boolean;
var
...
begin
  if ASegment.IsEmpty then
    raise EHefestoPeriodic.Create('Invalid empty cron segment.');

  LStep := 1;
  LStepParts := ASegment.Split(['/']);
  if Length(LStepParts) > 2 then
    raise EHefestoPeriodic.Create('Invalid cron step segment: ' + ASegment);
  ...
  begin
    ...
    if LStep <= 0 then
      raise EHefestoPeriodic.Create('Cron step must be greater than zero.');
  end;
  ...
  begin
    ...
    begin
      ...
      if LFinish < LStart then
        raise EHefestoPeriodic.Create('Cron range end cannot be lower than start.');
    end
    else
      raise EHefestoPeriodic.Create('Invalid cron range segment: ' + ASegment);
  end;
...
class function THefestoCronSchedule.Matches(
  const AExpression: string;
...
class function THefestoCronSchedule.ParseNumber(
  const AValue: string;
  const AMin, AMax: Integer;
...
class function THefestoCronSchedule.TruncateToMinute(
