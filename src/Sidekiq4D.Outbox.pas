...

{
  ...
  e as enfileira no Sidekiq4D.  Dessa forma, o job nunca se perde em caso
  de crash entre o commit e o Enqueue.

  Fluxo completo:
    1. Código de negócio abre transação no banco
    2. Insere/atualiza as tabelas de negócio
    3. Chama ISidekiqOutbox.Save(..., AConnection) — insere na tabela
       sidekiq_outbox dentro da mesma transação
    4. Commit — ambas as escritas (negócio + outbox) são atômicas
    5. ISidekiqOutboxPoller.PollOnce detecta a linha status='pending',
       chama Sidekiq4D.Enqueue e marca status='published'

  AConnection é TObject para manter esta unit livre de dependências
  FireDAC.  O adapter concreto (Sidekiq4D.Outbox.FireDAC) faz o cast
...
type
  ...
  ISidekiqOutboxPoller = interface
