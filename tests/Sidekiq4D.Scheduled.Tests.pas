unit Sidekiq4D.Scheduled.Tests;

{
  Testes unitários para TSidekiqInMemoryScheduledStore.
  Cobre: Schedule, Count, Clear, List, PopDue (limite, ordem, vencimento).
  Usa TSidekiqInMemoryScheduledStore — sem broker externo.
}
...
function TScheduledStoreTests.MakeEntry(
  ...
  const ADueAt: TDateTime): TSidekiqScheduledEntry;
begin
  Result := Default(TSidekiqScheduledEntry);
...
begin
  FStore := TSidekiqInMemoryScheduledStore.New;
end;
...
var
  LList: TArray<TSidekiqScheduledEntry>;
begin
  FStore.Schedule(MakeEntry('C', Now + 3));
  FStore.Schedule(MakeEntry('A', Now + 1));
  FStore.Schedule(MakeEntry('B', Now + 2));
