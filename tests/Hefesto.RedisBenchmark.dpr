program Hefesto.RedisBenchmark;

// Benchmark: individual commands vs Redis pipeline
// Fase 1 (ANTES): usa Redis4D diretamente
// Fase 2 (DEPOIS): usa IHefestoRedisClient.BeginPipeline (abstração Hefesto)
//
...
// ---------------------------------------------------------------------------
// DEPOIS: usando IHefestoRedisClient.BeginPipeline (abstração Hefesto)
// ---------------------------------------------------------------------------

procedure BenchHefestoLPushIndividual(
...
begin
  ...
  LWatch.Stop;
  AClient.Del(BENCH_KEY_LPUSH);
  PrintMetric('[Hefesto] RPush individual', ACount, LWatch.ElapsedMilliseconds);
end;

procedure BenchHefestoLPushPipeline(
  const AClient: IHefestoRedisClient;
  const ACount: Integer);
var
...
begin
  ...
  LWatch.Stop;
  AClient.Del(BENCH_KEY_LPUSH);
  PrintMetric('[Hefesto] LPush pipeline (BeginPipeline)', ACount, LWatch.ElapsedMilliseconds);
end;

procedure BenchHefestoZAddPipeline(
  const AClient: IHefestoRedisClient;
  const ACount: Integer);
var
  LPipeline: IHefestoRedisPipeline;
...
begin
  ...
  LWatch.Stop;
  AClient.Del(BENCH_KEY_ZADD);
  PrintMetric('[Hefesto] ZAdd pipeline (BeginPipeline)', ACount, LWatch.ElapsedMilliseconds);
end;

procedure BenchHefestoLLenBatch(
  const AClient: IHefestoRedisClient;
  const ACount: Integer);
var
  LKeys: TArray<string>;
...
begin
  ...
  LWatch.Stop;
  PrintMetric('[Hefesto] LLenBatch (pipeline)', ACount, LWatch.ElapsedMilliseconds);
end;

procedure BenchHefestoPooledLPushPipeline(ACount: Integer);
var
...
begin
  ...
  LWatch.Stop;
  LClient.Del(BENCH_KEY_LPUSH);
  PrintMetric('[Hefesto+Pool] LPush pipeline', ACount, LWatch.ElapsedMilliseconds);
end;
...
var
  ...
  LHefestoClient: IHefestoRedisClient;
  LN: Integer;
  LCounts: array[0..2] of Integer;
begin
  LClient := MakeClient;
  LHefestoClient := TRedis4DClientBridge.NewFromConnectionString(
  ...
  begin
    ...
    BenchHefestoLPushIndividual(LHefestoClient, LN);
    BenchHefestoLPushPipeline(LHefestoClient, LN);
    BenchHefestoPooledLPushPipeline(LN);
    Writeln;
  end;
  ...
  begin
    ...
    BenchHefestoZAddPipeline(LHefestoClient, LN);
    Writeln;
  end;
  ...
  begin
    ...
    BenchHefestoLLenBatch(LHefestoClient, LN);
    Writeln;
  end;
end;
...
begin
  try
    Writeln('Hefesto Redis Benchmark — ANTES e DEPOIS da implementacao de pipeline');
